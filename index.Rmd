...
color_dat <- tibble(
  colors = c("#0063af", "#009ce2", "#007557", "#e10600", "#F6531D", "#06778d", "#ffe200", "#00ADEF", "#914191", "#7EB82D", "#009B77", "#FCCB0A"),
...
library(gt)
library(gtExtras)
teams <- "https://github.com/favstats/provincies30/raw/main/provincies/2023-03-01.rds"
...
logos <- tibble(`Coalizione/Partito` = c("EKRE"),
       logo = c("https://upload.wikimedia.org/wikipedia/commons/0/0c/EKRE_logo.png"))

# debugonce(get_table_dat)
get_table_dat(election_dat30, party) %>% 
  arrange(desc(parse_number(`Total Spend`))) %>% 
  # left_join(logos) %>% 
  # select(Party = `Coalizione/Partito`, logo, everything()) %>% 
  gt(
    rowname_col = "Coalizione/Partito"
    # groupname_col = "group"
  ) %>% 
  fmt_markdown(columns = `Top Spenders`)  %>%
  cols_align(
    align = "center"
  ) %>%
  gtExtras::gt_theme_538() %>%
   ...
   # tab_options(data_row.padding = px(1))
  tab_options(table.width = pct(100))  %>%
    add_ribbons("EKRE", "#0063af") %>%
    add_ribbons("I", "#009ce2") %>%
    add_ribbons("K", "#007557") %>%
    add_ribbons("SDE", "#e10600") %>%
    # add_ribbons("REF", "#F6531D") %>%
    add_ribbons("E200", "#06778d") %>%
    add_ribbons("RE", "#ffe200") #%>%
    # add_ribbons("EER", "#00ADEF") %>%
    # add_ribbons("VE", "#914191") %>%
    # add_ribbons("EK", "#7EB82D") #%>%
    # add_ribbons("ROH", "#009B77") %>%
    # add_ribbons("TBA", "#FCCB0A")
  # tab_style(
  #   style = cell_borders(
  #     sides = c("left"),
  #     color = "#ef3e3e",
...
the_order <- contested_dat %>% 
  filter(party == "SDE") %>%   arrange(desc(perc)) %>% 
  pull(value) %>% 
  unique()
